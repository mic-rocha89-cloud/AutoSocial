...
function normalizeUploadResult(rawResult) {
  ...
  if (
    ...
    return {
      ...
      reason:
        "YouTube uploader returned an ambiguous result; remote effect is unknown.",
  ...
  const reason =
    rawResult?.reason ||
    rawResult?.error ||
    (ok ? "YouTube upload completed." : "YouTube upload failed.");
  return {
    ...rawResult,
    ok,
    outcome,
...
async function inspectStableVideoIdentity(filePath) {
  ...
  if (!before.isFile()) {
    throw new Error("YouTube uncertain source is not a regular file.");
  }
  ...
  if (!after.isFile() || !sameStableStat(before, after)) {
    throw new Error("YouTube uncertain source changed while it was inspected.");
  }
  const size = Number(after.size);
  if (!Number.isSafeInteger(size) || size < 0) {
    throw new Error("YouTube uncertain source size cannot be represented safely.");
  }
  return {
...
function getPreparedUncertainResult(metadata, preparationPath) {
  ...
  return {
    ...
    reason: valid
      ? metadata.reason
      : "YouTube has an invalid prepared uncertain state; manual reconciliation is required.",
    evidence: valid ? metadata.evidence : undefined,
    durableState: "uncertain-prepared",
    preparationPath,
    error:
      "YouTube has a prepared uncertain publication; no upload or retry was attempted.",
  };
}
...
async function findActiveClaimRecoveryResult(processingDir) {
  ...
  return {
    ...
    reason:
      "YouTube has an active durable queue claim; manual recovery is required before another upload.",
    durableState: "processing",
    error:
      "YouTube has an active durable queue claim; no upload or retry was attempted.",
  };
}
...
async function movePreparedUncertainVideo(videoPath, uncertainDir, prepared) {
  ...
  try {
    ...
    if (!sameVideoIdentity(prepared.videoIdentity, currentIdentity)) {
      throw new Error(
        "YouTube uncertain source identity changed before archival."
      );
    }
  } catch (error) {
    return {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "YouTube archived uncertain video does not match its prepared identity."
      );
    }
    return {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "YouTube quarantined uncertain video does not match its prepared identity."
        );
      }
      return {
        movedVideo: null,
...
async function postSingleVideo({
  ...
  try {
    ...
    return {
      ...
      retryAllowed: false,
      clickAttempted: false,
      reason: `YouTube claim snapshot integrity validation failed: ${error.message}`,
      error: `YouTube claim snapshot integrity validation failed: ${error.message}`,
      durableState: "processing",
    };
  }
  ...
  try {
    ...
    rawResult = {
      ...
      clickAttempted: null,
      reason:
        "YouTube uploader exited without a final result; remote effect is unknown.",
      error:
        "YouTube uploader exited without a final result; remote effect is unknown.",
    };
  }
  ...
  if (result.outcome === "uncertain") {
    ...
    let movedCaption = null;
    let sidecarsReady = sidecarPaths.length === 0;
    let sidecarError = null;
    if (transition.movedVideo && transition.identityVerified) {
      const movedCaptions = [];
      try {
        for (const sidecarPath of sidecarPaths) {
          movedCaptions.push(await moveWithTimestamp(sidecarPath, uncertain));
        }
        movedCaption = movedCaptions[0] || null;
        sidecarsReady = movedCaptions.length === sidecarPaths.length;
      } catch (error) {
        sidecarError = error.message;
      }
    }
    const terminalFilesReady = Boolean(
      transition.movedVideo && transition.identityVerified && sidecarsReady
    );
    const metadata =
      terminalFilesReady && preparation.prepared
        ? await writeUncertainMetadata(durableVideoPath, preparation.prepared)
        : { metadataPath: null, persistenceError: null };
    let cleanupError = null;
    if (metadata.metadataPath && preparation.preparationPath) {
      try {
    ...
    const fullyRecorded = Boolean(
      terminalFilesReady && metadata.metadataPath && !cleanupError
    );
    ...
    let finalized = {
      ...
      movedCaption,
      terminalFilesReady: fullyRecorded,
      durableState: fullyRecorded
        ? "uncertain"
...
async function postNextFromQueue({ source, queueDir, postedDir, failedDir, accountId } = {}) {
  ...
  try {
    ...
    return {
      ...
      clickAttempted: false,
      reason:
        "YouTube queue safety directories are unavailable; posting was aborted before selection.",
      error: `YouTube queue safety directories are unavailable: ${error.message}`,
    };
  }
  ...
  try {
    ...
    return {
      ...
      clickAttempted: false,
      reason:
        "YouTube queue recovery state could not be inspected; posting was aborted.",
      error: `YouTube queue recovery state could not be inspected: ${error.message}`,
    };
  }
  ...
  try {
    ...
    return {
      ...
      retryAllowed: false,
      clickAttempted: false,
      reason: `Could not bind the selected YouTube queue item: ${error.message}`,
      error: `Could not bind the selected YouTube queue item: ${error.message}`,
    };
  }
  if (!nextItem) {
  ...
  try {
    ...
    return {
      ...
      clickAttempted: false,
      reason: `Could not atomically claim the queued YouTube video: ${error.message}`,
      error: `Could not atomically claim the queued YouTube video: ${error.message}`,
    };
  }
...
async function postFromManualInput() {
  const reason =
    "Direct YouTube posting by file path is disabled. " +
    "Place the video in the YouTube pending queue and use the managed queue lifecycle.";
  return {
    ok: false,
    skipped: false,
    outcome: "failure",
